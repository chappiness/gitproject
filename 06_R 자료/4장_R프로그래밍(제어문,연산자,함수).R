...
matrix(c(nums, result), ncol = 5, byrow = TRUE,
...
fact(7)
rm(list=ls(all.names = TRUE)) # 히든변수까지 다 삭제
fact(7)
...
##### 연습문제. 매개변수가 소수(Prime Number)인지 여부 return #####
is.prime.number <- function(num){
  flag <- TRUE
  if(num<=1){
    cat('소수 고려 대상은 1보다 큰 수입니다.')
  }else if(num==2){
    return (TRUE)
  }else{
    for(i in 2:(num-1)){
